...
- $$M_{S_k}=E\left[\left|S_k\right|^2\right]$$

- $$M_{\hat{S}_k^{(l-1)}}=E\left[\left|\hat{S}_k^{(l-1)}\right|^2\right]$$

- $$M_W=E\left[\left|W_k\right|^2\right]$$

- $$r_{S_k, \hat{S}_k^{(l-1)}}=E\left[S_k \hat{S}_k^{(l-1) *}\right]$$

代入并简化期望项后：

$$
\begin{aligned}
E\left[\left|e_k\right|^2\right]= & \left|C_k H_k-1\right|^2 M_{S_k}+\left|C_k\right|^2 M_W+\left|B_k\right|^2 M_{\hat{S}_k^{(l-1)}} \\
& +2 \Re\left[B_k^*\left(C_k H_k-1\right) r_{S_k, \hat{S}_k^{(l-1)}}\right]
\end{aligned}
$$
